...
namespace {
	...
	void addLine(sf::VertexArray& vertices,
		...
		vertices.append(
		  sf::Vertex(sf::Vector2f(-outlineThickness, top - outlineThickness),
		             color,
		             sf::Vector2f(1, 1)));
		...
		vertices.append(sf::Vertex(
		  ...
		  color,
		  sf::Vector2f(1, 1)));
		vertices.append(sf::Vertex(sf::Vector2f(lineLength + outlineThickness,
	...
	void addGlyphQuad(sf::VertexArray& vertices,
		...
		vertices.append(sf::Vertex(
		  sf::Vector2f(position.x + left - italic * top - outlineThickness,
		               position.y + top - outlineThickness),
		  color,
		  sf::Vector2f(u1, v1)));
		vertices.append(sf::Vertex(
		  sf::Vector2f(position.x + right - italic * top - outlineThickness,
		               position.y + top - outlineThickness),
		  color,
		  sf::Vector2f(u2, v1)));
		vertices.append(sf::Vertex(
		  sf::Vector2f(position.x + left - italic * bottom - outlineThickness,
		...
		vertices.append(sf::Vertex(
		  ...
		  sf::Vector2f(u2, v1)));
		vertices.append(sf::Vertex(
		  sf::Vector2f(position.x + right - italic * bottom - outlineThickness,
		               position.y + bottom - outlineThickness),
		  color,
		  sf::Vector2f(u2, v2)));
	}
...
namespace sf {
	...
	////////////////////////////////////////////////////////////
	MulticolourText::MulticolourText(const String& string,
	                                 const Font&   font,
	...
	////////////////////////////////////////////////////////////
	void MulticolourText::setFillColor(const Color& color,
	                                   size_t       _startPos,
	                                   size_t       _endPos) {
		if (_endPos == 0) {
	...
	////////////////////////////////////////////////////////////
	void MulticolourText::setOutlineColor(const Color& color,
	                                      size_t       _startPos,
	                                      size_t       _endPos) {
		if (_endPos == 0) {
			_endPos = m_string.getSize() - 1;
	...
	////////////////////////////////////////////////////////////
	void MulticolourText::setOutlineThickness(float  thickness,
	                                          size_t _startPos,
	                                          size_t _endPos) {
		if (_endPos == 0) {
			_endPos = m_string.getSize() - 1;
	...
	Vector2f MulticolourText::findCharacterPos(std::size_t index) const {
		...
		float vspace =
		  static_cast<float>(m_font->getLineSpacing(m_characterSize));
		...
		for (std::size_t i = 0; i < index; ++i) {
			...
			position.x += static_cast<float>(
			  m_font->getKerning(prevChar, curChar, m_characterSize));
			prevChar = curChar;
			...
			position.x += static_cast<float>(
			  m_font->getGlyph(curChar, m_characterSize, bold).advance);
		}
	...
	////////////////////////////////////////////////////////////
	void MulticolourText::draw(RenderTarget& target,
	...
	void MulticolourText::ensureGeometryUpdate() const {
		...
		FloatRect xBounds =
		  m_font->getGlyph(L'x', m_characterSize, bold).bounds;
		...
		for (std::size_t i = 0; i < m_string.getSize(); ++i) {
			...
			// Apply the outline
			if (curOutlineThickness != 0) {
				const Glyph& glyph = m_font->getGlyph(
				  curChar, m_characterSize, bold, curOutlineThickness);
				...
				minX = std::min(
				  minX, x + left - italic * bottom - curOutlineThickness);
				maxX = std::max(maxX,
				                x + right - italic * top - curOutlineThickness);
			...
			const Glyph& glyph =
			  m_font->getGlyph(curChar, m_characterSize, bold);

			// Add the glyph to the vertices
			addGlyphQuad(
			  m_vertices, Vector2f(x, y), curFillColor, glyph, italic);
			m_vertexIndeces.insert(
			  std::make_pair(i, m_vertices.getVertexCount() - 6));

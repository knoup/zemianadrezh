...
void RendererPlayer::getInput(sf::Event& _event) {
    switch(_event.type) {
        case sf::Event::Resized: {
            onResize({float(_event.size.width),
					float(_event.size.height)});
            break;
        }
...
void RendererPlayer::onResize(sf::Vector2f _newSize) {
	m_view.setSize(_newSize);
}
